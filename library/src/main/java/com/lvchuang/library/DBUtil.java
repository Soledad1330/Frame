...
import com.lvchuang.library.db.config.DaoConfig;
import com.lvchuang.library.db.entitiy.DBModel;
import com.lvchuang.library.db.entitiy.KeyValue;
import com.lvchuang.library.db.entitiy.ManyToOne;
import com.lvchuang.library.db.entitiy.OneToMany;
import com.lvchuang.library.db.entitiy.SqlInfo;
import com.lvchuang.library.db.entitiy.TableInfo;
import com.lvchuang.library.db.utils.CursorHelper;
import com.lvchuang.library.db.utils.OneToManyLazyLoader;
import com.lvchuang.library.db.utils.SqlBuilder;
...
public class DBUtil {
    ...
    private DBUtil(DaoConfig config) {
        ...
        if (config.getTargetDirectory() != null && config.getTargetDirectory().trim().length() > 0) {
            ...
            this.db = new SqliteDBUtil(config.getContext().getApplicationContext(), config.getDbName(),
     ...
     * @param dbName  数据库名称
     */
    public static DBUtil create(Context context, String targetDirectory, String dbName) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
        config.setDbName(dbName);
     ...
     * @param isDebug 是否为debug模式（debug模式进行数据库操作的时候将会打印sql语句）
     */
    public static DBUtil create(Context context, String targetDirectory, String dbName, boolean isDebug) {
        DaoConfig config = new DaoConfig();
        config.setContext(context);
        config.setTargetDirectory(targetDirectory);
        config.setDbName(dbName);
     ...
     * @return
     */
    public static DBUtil create(Context context, String dbName, boolean isDebug, int dbVersion,
    ...
    private void insertContentValues(List<KeyValue> list, ContentValues cv) {
        if (list != null && cv != null) {
            ...
            LogUtil.debug(
                    getClass().getName() + "insertContentValues: List<KeyValue> is empty or ContentValues is empty!");
        }
    ...
    private void exeSqlInfo(SqlInfo sqlInfo) {
        if (sqlInfo != null) {
            ...
            LogUtil.debug(getClass().getName() + "sava error:sqlInfo is null");
        }
    }
    ...
    private class SqliteDBUtil extends SQLiteOpenHelper {

        private final DbUpdateListener mDbUpdateListener;

        public SqliteDBUtil(Context context, String name, int version, DbUpdateListener dbUpdateListener) {
